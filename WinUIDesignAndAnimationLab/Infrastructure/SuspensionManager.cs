...
internal sealed class SuspensionManager
{
    ...
    private static readonly DependencyProperty FrameSessionBaseKeyProperty =
        DependencyProperty.RegisterAttached("_FrameSessionBaseKeyParams", typeof(string), typeof(SuspensionManager),
            null);
    ...
    /// </summary>
    public static List<Type> KnownTypes { get; } = new();
    ...
    public static async Task SaveAsync()
    {
        try
        {
            ...
            {
                Frame frame;
                if (weakFrameReference.TryGetTarget(out frame)) SaveFrameNavigationState(frame);
            }
    ...
    private static void RestoreFrameNavigationState(Frame frame)
    {
        ...
        if (frameState.ContainsKey("Navigation")) frame.SetNavigationState((string)frameState["Navigation"]);
    }
