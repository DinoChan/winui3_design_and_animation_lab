...
public sealed partial class GlitchText3 : UserControl
{
    public GlitchText3()
    {
        ...
        Loaded += async (s, e) =>
        {
            ...
            StartScaleAnimation(redMaskBrush,
                new List<(float, float)> { (0, 0.01f), (.20f, .73f), (.60f, .14f), (1, .95f) }, TimeSpan.FromSeconds(1),
                TimeSpan.Zero);
            StartScaleAnimation(blueMaskBrush,
                new List<(float, float)>
                    { (0, 1), (.20f, 1), (.35f, .27f), (.50f, .91f), (.60f, .45f), (.70f, .77f), (.80f, .5f), (1, 0) },
                TimeSpan.FromSeconds(1.5), TimeSpan.Zero);
    ...
    private void StartScaleAnimation(CompositionSurfaceBrush brush, List<(float, float)> keyFrames, TimeSpan duration,
        TimeSpan delay)
    {
